...
/* Criar a classe CategoryRemoteDataSource para o retrofit fazer a req no servidor baseado
...
class CategoryRemoteDataSource {
    fun findAllCategories(callback: ListCategoryCallback) {
        HTTPClient.retrofit() //aqui tem a instancia do retrofit
            ...
            .findAllCategories()                  //fun da interface
            .enqueue(object: Callback<List<String>> {   //enfileirar e fazer que a execução seja assincrona
                          // vai devolver um Callback com mesmo tipo de ret da interface e response com ok ou erro

                override fun onResponse(call: Call<List<String>>,response: Response<List<String>>){
                    if (response.isSuccessful) {
                        val categories = response.body()    //No response tem a Lista de categorias
